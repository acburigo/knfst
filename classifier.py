import numpy as np
import scipy as sp
from learn_multiclass_novelty import learn_multiclass_novelty
from learn_oneclass_novelty import learn_oneclass_novelty
...
class KernelNullSpaceClassifier(object):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        proj, target_points = learn_multiclass_novelty(kernel_mat, y)
...
class OneClassKernelNullSpaceClassifier(object):
    def __init__(self, threshold=0.5, metric='hik'):
        self.metric = self._get_metric(metric)
        self.threshold = threshold
    ...
    def fit(self, X, y=None, sample_weight=None):
        kernel_mat = metrics.pairwise_kernels(X, metric=self.metric)
        proj, target_points = learn_oneclass_novelty(kernel_mat)
